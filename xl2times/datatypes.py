...
class TimesModel:
    ...
    def past_years(self) -> set[int]:
        ...
        """
        if self.attributes.empty or "attribute" not in self.attributes.columns:
            return set()
        i = self.attributes["attribute"] == "NCAP_PASTI"
        if "year" not in self.attributes.columns:
            return set()
    ...
    def model_years(self) -> set[int]:
        ...
        """
        if self.time_periods.empty or "m" not in self.time_periods.columns:
            return self.past_years
